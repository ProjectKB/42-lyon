...
static void	transform_buffer(t_sha256 *sha256, __uint32_t *buf, int i)
{
	...
	t1 = buf[7] + sig1(buf[4]) + ch(buf[4], buf[5], buf[6]) + \
													g_k[i] + sha256->words[i];
	t2 = sig0(buf[0]) + maj(buf[0], buf[1], buf[2]);
	buf[7] = buf[6];
	buf[6] = buf[5];
	buf[5] = buf[4];
	buf[4] = buf[3] + t1;
...
static void	special_process(t_sha256 *sha256)
{
	...
	k = 15;
	while (++k < 64)
		sha256->words[k] = lsig1(sha256->words[k - 2]) + sha256->words[k - 7] + \
		lsig0(sha256->words[k - 15]) + sha256->words[k - 16];
...
void		proceed_block_sha256(t_hash *h)
{
	...
	if (i == 64)
	{
		...
		k = 15;
		while (++k < 64)
			h->sha256.words[k] = lsig1(h->sha256.words[k - 2]) + \
			h->sha256.words[k - 7] + lsig0(h->sha256.words[k - 15]) + \
			h->sha256.words[k - 16];
...
void		proceed_last_block_sha256(t_hash *h)
{
	...
	i = 15;
	while (++i < 64)
		h->sha256.words[i] = lsig1(h->sha256.words[i - 2]) + \
		h->sha256.words[i - 7] + lsig0(h->sha256.words[i - 15]) + \
		h->sha256.words[i - 16];
	transform_block(&h->sha256);
}
